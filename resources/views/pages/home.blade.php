...
@section('seo_description', 'Ветеранський простір у Хмельницькому - підтримка ветеранів, військових та їхніх родин.
Соціальна адаптація, психологічна допомога, юридичні консультації.')
@section('content')
...
@if(\App\Models\NewsArticle::published()->count() >= 4)
...
<div class="hidden lg:block w-full mb-[300px]">
    ...
    <!-- News Grid -->
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[55px]">
        @foreach($news as $article)
        <a href="{{ route('news.show', $article) }}" class="flex flex-col group block">
            <!-- Image -->
            ...
            <div class="w-full mt-auto pt-20 flex justify-between items-center">
                ...
                <div
                    ...
                    <span
                        class="text-black dark:text-white group-hover:text-white text-xs font-bold uppercase leading-none tracking-wide transition-colors">{{
                        __('Read More') }}</span>
                </div>
            </div>
        </a>
...
<div class="hidden lg:block mt-[100px] xl:mt-[152px] mb-[200px] xl:mb-[300px] px-8 xl:px-0">
    <div class="relative w-full max-w-[1170px] mx-auto">
        ...
        <div class="flex justify-between items-start mb-12">
            ...
            <!-- Team Button -->
            <a href="{{ route('team') }}"
                class="w-48 xl:w-64 h-14 xl:h-16 mt-2 border-2 border-veteran-blue bg-white dark:bg-zinc-900 text-black dark:text-white text-lg xl:text-xl font-black font-montserrat uppercase tracking-wide cursor-pointer transition-all duration-300 ease-in-out rounded-[412px] flex items-center justify-center hover:bg-veteran-blue hover:text-white shrink-0">
                {{ __('Team') }}
            </a>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const totalItems = {{ $partners-> count()
